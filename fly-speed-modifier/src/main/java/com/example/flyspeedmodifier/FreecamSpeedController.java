...
public final class FreecamSpeedController {
	...
	public static boolean handleMouseScroll(double verticalScroll) {
		...
		temporaryMultiplier = adjustMultiplierNonLinearly(
				temporaryMultiplier,
				verticalScroll,
				FlySpeedModifierConfig.scrollStep()
		);
	...
	private static void applyDirectFlyingSpeed(Minecraft client) {
		...
		client.player.getAbilities().setFlyingSpeed((float) (directOriginalFlyingSpeed * temporaryMultiplier));
	}
	...
	private static double adjustMultiplierNonLinearly(double currentMultiplier, double scrollAmount, double scrollStep) {
	...
	private static void showMultiplierOverlay(SpeedTarget target, double multiplier) {
		...
		String label = target == SpeedTarget.FREECAM ? "Freecam speed multiplier" : "Flight speed multiplier";

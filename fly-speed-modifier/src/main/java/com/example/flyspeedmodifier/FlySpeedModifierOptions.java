...
public final class FlySpeedModifierOptions {
	private static final OptionInstance<Boolean> FULL_RANGE = OptionInstance.createBoolean(
			"option.fly-speed-modifier.full_range",
			value -> Tooltip.create(Component.translatable("option.fly-speed-modifier.full_range.tooltip")),
			FlySpeedModifierConfig.DEFAULT_FULL_RANGE,
			FlySpeedModifierOptions::onFullRangeChanged
	);

	private static final OptionInstance<Double> MAX_SPEED = new OptionInstance<>(
			"option.fly-speed-modifier.max_speed",
			value -> Tooltip.create(Component.translatable("option.fly-speed-modifier.max_speed.tooltip")),
	...
	private static final OptionInstance<Double> MIN_SPEED = new OptionInstance<>(
			"option.fly-speed-modifier.min_speed",
			value -> Tooltip.create(Component.translatable("option.fly-speed-modifier.min_speed.tooltip")),
			FlySpeedModifierOptions::speedText,
			UnitDouble.INSTANCE.xmap(
	...
	private static final OptionInstance<Double> INITIAL_SPEED = new OptionInstance<>(
			"option.fly-speed-modifier.initial_speed",
			value -> Tooltip.create(Component.translatable("option.fly-speed-modifier.initial_speed.tooltip")),
			FlySpeedModifierOptions::speedText,
			UnitDouble.INSTANCE.xmap(
	...
	private static final OptionInstance<Boolean> RESET_ON_ADJUST = OptionInstance.createBoolean(
			"option.fly-speed-modifier.reset_on_adjust",
			value -> Tooltip.create(Component.translatable("option.fly-speed-modifier.reset_on_adjust.tooltip")),
			FlySpeedModifierConfig.DEFAULT_RESET_ON_ADJUST,
			FlySpeedModifierOptions::onResetOnAdjustChanged
	);

	private static final OptionInstance<Double> SCROLL_STEP = new OptionInstance<>(
			"option.fly-speed-modifier.scroll_step",
			value -> Tooltip.create(Component.translatable("option.fly-speed-modifier.scroll_step.tooltip")),
	...
	private static double toMaxSpeed(double normalized) {
		double upperBound = FULL_RANGE.get()
				? FlySpeedModifierConfig.FULL_RANGE_MAX_SPEED
				: FlySpeedModifierConfig.STANDARD_MAX_SPEED;
		double result = FlySpeedModifierConfig.MIN_ALLOWED_MAX_SPEED
	...
	private static double fromMaxSpeed(double speed) {
		double upperBound = FULL_RANGE.get()
				? FlySpeedModifierConfig.FULL_RANGE_MAX_SPEED
				: FlySpeedModifierConfig.STANDARD_MAX_SPEED;
		double clamped = Math.max(FlySpeedModifierConfig.MIN_ALLOWED_MAX_SPEED, Math.min(speed, upperBound));
	...
	private static double toInitialSpeed(double normalized) {
		...
		return roundToTwoDecimals(
				lowerBound + normalized * (upperBound - lowerBound)
		);
	}
	...
	private static double toScrollStep(double normalized) {
		double range = FlySpeedModifierConfig.MAX_ALLOWED_SCROLL_STEP - FlySpeedModifierConfig.MIN_ALLOWED_SCROLL_STEP;
	...
	private static void constrainInitialSpeed(FlySpeedModifierConfigScreen screen) {
		double lowerBound = screen.draft().minSpeed;
		double upperBound = effectiveMaximumSpeed(screen);
		double constrained = Math.max(
				lowerBound,
				Math.min(screen.draft().initialSpeed, upperBound)
		);
	...
	private static double effectiveMaximumSpeed(FlySpeedModifierConfigScreen screen) {
		double fullRangeUpperBound = screen.draft().fullRange
				? FlySpeedModifierConfig.FULL_RANGE_MAX_SPEED
				: FlySpeedModifierConfig.STANDARD_MAX_SPEED;
		return Math.max(
				screen.draft().minSpeed,
				Math.min(screen.draft().maxSpeed, fullRangeUpperBound)
		);
	}

	private static Component speedText(Component optionText, Double value) {
		return Component.literal(optionText.getString() + ": " + FlySpeedModifierConfig.formatSpeed(value));
	}

	private static Component scrollStepText(Component optionText, Double value) {
		return Component.literal(optionText.getString() + ": " + FlySpeedModifierConfig.formatScrollStep(value));
	}

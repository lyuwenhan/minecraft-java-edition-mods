...
public final class NoHungryMod implements ModInitializer {
    ...
    private static void registerCommands(com.mojang.brigadier.CommandDispatcher<net.minecraft.commands.CommandSourceStack> dispatcher) {
        ...
        minHungerValue.executes(context -> {
            int value = IntegerArgumentType.getInteger(context, "value");
            config.setFoodMinimum(value);
            config.save();
            context.getSource().sendSuccess(() -> Component.literal("Minimum hunger level set to " + value + "."), true);
            return 1;
        });
        ...
        minSaturationValue.executes(context -> {
            int value = IntegerArgumentType.getInteger(context, "value");
            config.setSaturationMinimum(value);
            config.save();
            context.getSource().sendSuccess(() -> Component.literal("Minimum saturation level set to " + value + "; hunger level fixed at 20."), true);
            return 1;
        });
        ...
        toggleCommand.executes(context -> {
            boolean enabled = config.toggleEnabled();
            config.save();
            context.getSource().sendSuccess(() -> Component.literal("No Hungry " + (enabled ? "enabled" : "disabled") + "."), true);
            return 1;
        });
        rootCommand.then(toggleCommand);

        var statusCommand = Commands.literal("status");
        statusCommand.executes(context -> {
    ...
    private static String getStatusMessage() {
        ...
        return "No Hungry enabled.\nminimum hunger: " + minimumFoodLevel + "\nminimum saturation: " + minimumSaturationLevel;
    }
}
